...
if $position_dependent_phones; then
    ...
    cat <(
        set -f
        for x in $(cat $srcdir/silence_phones.txt); do
            for y in "" "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done
            echo
        done
    ) \
        <(
            set -f
            for x in $(cat $srcdir/nonsilence_phones.txt); do
                for y in "" "_B" "_E" "_I" "_S"; do echo -n "$x$y "; done
                echo
            done
        ) \
...
if $position_dependent_phones; then
    for suffix in _B _E _I _S; do
        (
            set -f
            for x in $(cat $srcdir/nonsilence_phones.txt); do echo -n "$x$suffix "; done
            echo
        ) >> $dir/phones/extra_questions.txt
    done
    for suffix in "" _B _E _I _S; do
        (
            set -f
            for x in $(cat $srcdir/silence_phones.txt); do echo -n "$x$suffix "; done
            echo
        ) >> $dir/phones/extra_questions.txt
    done
fi
...
if $silprob; then
    ...
    utils/lang/make_subword_lexicon_fst.py $grammar_opts --sil-prob=$sil_prob --sil-phone=$silphone --position-dependent --separator=$separator $tmpdir/lexiconp.txt \
