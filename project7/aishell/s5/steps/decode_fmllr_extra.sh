...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
    ...
    if [ $stage -le 0 ]; then
        ...
        steps/decode.sh --acwt $acwt --nj $nj --cmd "$cmd" --beam $first_beam --model $alignment_model --max-active $first_max_active --num-threads $num_threads --skip-scoring true $graphdir $data $si_dir || exit 1
    fi
fi
##
...
if [ $stage -le 2 ]; then
    ...
    $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode1.JOB.log gmm-latgen-faster$thread_string --max-active=$first_max_active --max-mem=$max_mem --beam=$first_beam --lattice-beam=$first_lattice_beam \
...
if [ $stage -le 4 ]; then
    ...
    $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode2.JOB.log gmm-latgen-faster$thread_string --max-active=$max_active --max-mem=$max_mem --beam=$beam --lattice-beam=$lattice_beam \
        --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt \
