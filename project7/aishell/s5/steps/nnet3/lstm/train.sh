...
case $feat_type in
    raw)
        feat_dim=$(feat-to-dim --print-args=false scp:$data/feats.scp -) \
            || {
                echo "$0: Error getting feature dim"
                exit 1
            }
        ;;
    lda)
        ...
        ;;
    *)
        echo "$0: Bad --feat-type '$feat_type';"
        exit 1
        ;;
esac
...
chunk_width=$(cat $egs_dir/info/frames_per_eg) || {
    echo "error: no such file $egs_dir/info/frames_per_eg"
    exit 1
}
num_archives=$(cat $egs_dir/info/num_archives) || {
    echo "error: no such file $egs_dir/info/num_archives"
    exit 1
}
...
while [ $x -lt $num_iters ]; do
    ...
    ilr=$initial_effective_lrate
    flr=$final_effective_lrate
    np=$num_archives_processed
