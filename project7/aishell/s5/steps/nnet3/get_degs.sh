...
[ -f $dir/ali.scp ] || {
    echo "$0: expected $dir/ali.scp to exist"
    exit 1
}
...
if ! [ "$num_frames" -gt 0 ]; then
    echo "$0: bad num-frames=$num_frames"
    exit 1
fi
...
if [ $num_threads -eq 1 ]; then
    ...
    $looped && {
        echo "$0: --num-threads must be one if you use looped decoding"
        exit 1
    }
...
if [ $stage -le 4 ]; then
    ...
    for j in $(seq $nj); do
        cat $dir/degs_valid.$j.scp
        rm $dir/degs_valid.$j.scp
    done | utils/shuffle_list.pl | head -n$num_utts_subset > $dir/valid_diagnostic.scp || exit 1

    [ -s $dir/valid_diagnostic.scp ] || {
        echo "$0: error getting validation egs"
        exit 1
    }
fi
...
if [ $stage -le 6 ]; then
    ...
    for n in $(seq $num_archives); do
        head -n$nl $dir/degs.$n.scp
        tail -n$nl $dir/degs.$n.scp
    done | utils/shuffle_list.pl | head -n$num_utts_subset > $dir/train_diagnostic.scp
    [ -s $dir/train_diagnostic.scp ] || {
        echo "$0: error getting train_diagnostic.scp"
        exit 1
    }
fi
...
if [ $stage -le 10 ] && $cleanup; then
    ...
    for j in $(seq $nj); do
        for f in $dir/degs_orig.$j.{ark,scp} $dir/degs_orig_filtered.$j.scp; do
            [ -L $f ] && rm $(utils/make_absolute.sh $f)
            rm $f
        done
    done
