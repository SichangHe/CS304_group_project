...
while [ $x -lt $num_iters ]; do
    if [ $stage -le $x ]; then
        ...
        [ -f $dir/.error ] && {
            echo "Found $dir/.error. See $dir/log/train.$x.*.log"
            exit 1
        }
        ...
        [ -f $dir/.error ] && {
            echo "Found $dir/.error. Error on iteration $x"
            exit 1
        }
    fi
    ...
    if [ $stage -le $x ] && [ ! -z "${iter_to_epoch[$x]}" ]; then
        ...
        (
            ...
            steps/nnet3/adjust_priors.sh --egs-type degs \
                ...
                || {
                    touch $dir/.error
                    echo "Error in adjusting priors. See errors above."
                    exit 1
                }
        ) &
    fi
