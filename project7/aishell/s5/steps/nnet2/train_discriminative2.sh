    ...
    if $adjust_priors && [ ! -z "${iter_to_epoch[$x]}" ]; then
        ...
        (
            ...
            num_archives_priors=$(cat $degs_dir/info/num_archives_priors) || {
                touch $dir/.error
                echo "Could not find $degs_dir/info/num_archives_priors. Set --adjust-priors false to not adjust priors"
                exit 1
            }
            ...
            $cmd JOB=1:$num_archives_priors $dir/log/get_post.epoch$e.JOB.log \
                ...
                || {
                    touch $dir/.error
                    echo "Error in getting posteriors for adjusting priors. See $dir/log/get_post.epoch$e.*.log"
                    exit 1
                }
            ...
            $cmd $dir/log/sum_post.epoch$e.log \
                ...
                || {
                    touch $dir/.error
                    echo "Error in summing posteriors. See $dir/log/sum_post.epoch$e.log"
                    exit 1
                }
            ...
            $cmd $dir/log/adjust_priors.epoch$e.log \
                ...
                || {
                    touch $dir/.error
                    echo "Error in adjusting priors. See $dir/log/adjust_priors.epoch$e.log"
                    exit 1
                }
        ) &
    fi
...
remove() { for x in $*; do
    [ -L $x ] && rm $(utils/make_absolute.sh $x)
    rm $x
done; }
